...
| 0020 | `fecha_fin_eventos_y_cotizador` | Fecha/hora de término de eventos (multi-día), datos bancarios del cotizador separados, cupo máximo |
| 0021 | `estado_anticipo_cotizacion` | Nuevo estado "anticipo" en cotizaciones |
| 0022 | `carta_responsiva_y_contenido_cotizacion` | Campo de película/contenido, y seguimiento del envío de la Carta Responsiva |
| 0023 | `rol_taquilla_restringido` | Nuevo rol "taquilla" — solo Taquilla en vivo, sin acceso a cotizaciones ni encuestas |
| 0024 | `restringir_edicion_eventos_taquilla` | Separa lectura (cualquier staff) de escritura (solo admin) sobre eventos |
| 0025 | `fecha_fin_cotizaciones` | Fecha de término para cotizaciones multi-día (no aplica a Sala Cinemátika) |
| 0026 | `clasificacion_cotizaciones` | Clasificación de edad del contenido/evento cotizado (AA/A/B/B-15/C/D) |
| 0027 | `concepto_y_beneficiario_cotizaciones` | Concepto de captura manual (sin "Renta"), y Beneficiario para el PDF |

## Edge Functions desplegadas (actualización)

| Función | Qué hace |
|---|---|
| `enviar-carta-responsiva` | Envía la Carta Responsiva (PDF generado en el navegador) por correo con el pago adjunto — solo admins, disparada al marcar Anticipo o Pagado |
...
| `evento-preview` | Genera etiquetas Open Graph por evento (imagen/título del banner) para que compartir en WhatsApp/Facebook/X muestre una vista previa correcta |
| `enviar-encuesta` | Envía la Encuesta de Calidad a los asistentes reales de un evento presencial ya concluido — solo la puede llamar un admin autenticado (verify_jwt=true) |
